...
#imputs
raiz = "X:/Sigmagis/Projetos/GRUPO TEREOS/NDVI/2025/J2/MANDU"
shp = 'BASE_TALHOES_NDVI_MANDU_J2_2025.shp'
ervas= 'ERVAS_MANDU_J2_2025.shp'
...
def processo():
    ...
    merge_sem_erva = os.path.join(MERGE_ERVAS,'merge_sem_erva.shp')
    dissolvido_salva.to_file(merge_sem_erva)
...
while executa:
    ...
    if opcao == 'N':
        ...
        if ervas.endswith('.shp'):
            ...
            saida_dif = os.path.join(MERGE_ERVAS,'saida_dif_erva.shp')
            ...
            for di in os.listdir(MERGE):
                if di.endswith('.shp'):
                    ...
                    erva = erva.dropna(subset=["geometry"])  # Remove geometrias None antes de processar
                    ...
                    print("Diferença shape intersect com erva!\n")
                    dif= dissolv.overlay(ervas_dissolvidas, how='difference') #dissolve 
                    dif['GRIDCODE']=dif['GRIDCODE'].astype(int)
                    dif.to_file(saida_dif)
                    ...
                    if not intersect_final['GRIDCODE'].apply(float.is_integer).all():
